<script lang="ts">
  ...
  const NAV = [
    { href: "/", label: "Overview" },
    { href: "/build", label: "Build a Shimpz" },
...
<div class="wrap flex flex-col gap-8 pt-10 pb-16 lg:flex-row">
  <aside class="lg:w-52 lg:shrink-0">
    <div class="lg:sticky lg:top-24">
      <span class="kicker">Documentation</span>
      <nav class="mt-3 flex flex-wrap gap-1.5 lg:flex-col">
        {#each NAV as item (item.href)}
          <a href={item.href} class="rounded-lg px-3 py-2 text-sm transition hover:text-[var(--color-fg)]"
             style={active(item.href) ? "background:var(--color-elevated);color:var(--color-fg);font-weight:600" : "color:var(--color-muted)"}>{item.label}</a>
        {/each}
      </nav>
    </div>
  </aside>

  <main class="md min-w-0 flex-1 max-w-3xl">
    {@render children()}
  </main>
</div>
