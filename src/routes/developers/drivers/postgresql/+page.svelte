...
<svelte:head>
  <title>PostgreSQL Driver — Shimpz docs</title>
  <meta
    name="description"
    content="The first Driver Spec v1 implementation: one PostgreSQL control service with exact Capsule and App database scope."
  />
</svelte:head>

<nav class="docs-breadcrumb" aria-label="Breadcrumb">
  <a href="/developers/drivers/">Drivers</a><span aria-hidden="true">/</span><strong>PostgreSQL</strong>
</nav>

<header class="docs-page-header">
  <span class="section-label">Conforming reference · Driver Spec v1</span>
  <h1>PostgreSQL Driver</h1>
  <p class="docs-lede">
...
<section class="guide-section" aria-labelledby="postgres-boundary-title">
  ...
  <h2 id="postgres-boundary-title">The control credential stops at the Driver</h2>
  <ul>
    <li>Only the PostgreSQL Driver holds the administrator DSN.</li>
    <li>The provisioner uses a dedicated Bearer token for Capsule creation and finalization.</li>
    <li>Each Capsule receives a random principal; the Driver stores its hash and exact database set.</li>
    <li>Database and role names are derived on the server from validated Capsule and App identifiers.</li>
    <li>Apps connect directly with their scoped database URL after provisioning.</li>
...
<section class="guide-section" aria-labelledby="postgres-discovery-title">
  ...
  <CodeBlock
    label="PostgreSQL Driver discovery and protected control routes"
  ...
  />
  <p>
    The two <code>GET</code> routes reveal only liveness and validated manifest metadata. Every
...
<section class="guide-section" aria-labelledby="postgres-proof-title">
  ...
  <ul>
    ...
    <li>Provision, retry, App teardown, Capsule retirement, and legacy-route denial retain their isolation tests.</li>
  </ul>
  <p>
    Read the <a
      class="external-link"
      ...
      aria-label="PostgreSQL Driver source on GitHub (opens in a new tab)">PostgreSQL Driver source</a
    >.
  </p>
</section>
...
<aside class="scope-note" aria-labelledby="delivery-boundary-title">
  ...
  <p>
    ...
    Admin installer can install this Driver or that a standalone Driver image has been released.
  </p>
</aside>
