...
<svelte:head>
  <title>Power request lifecycle and security — Shimpz docs</title>
  <link rel="canonical" href="https://docs.shimpz.com/developers/assistants/requests/lifecycle/" />
  <meta
    name="description"
    content="Design deterministic Power request replay, handle denial, and keep credentials at the last boundary."
  />
</svelte:head>
...
<header class="docs-page-header">
  ...
  <p class="docs-lede">
    A human request does not keep untrusted code suspended in memory. It ends the Power process. After a valid human
    response, Shimpz invokes the same immutable Power again and the SDK deterministically replays completed requests.
  </p>
</header>

<section class="guide-section" aria-labelledby="sequence-title">
  <span class="section-label">What actually happens</span>
  <h2 id="sequence-title">One logical Power may use several short processes</h2>
  <ol>
    <li>The Controller invokes the reviewed Power with its original input and no response transcript.</li>
...
<section class="guide-section" aria-labelledby="ordering-title">
  ...
  <CodeBlock label="Replay-safe Power shape" title="powers/publish_record.py" variant="code" {...data.safeOrder} />
  <p>
...
<section class="guide-section" aria-labelledby="determinism-title">
  ...
  <ul>
    <li>Build request copy and options from original Power inputs or already replayed responses.</li>
...
<section class="guide-section" aria-labelledby="terminal-title">
  ...
  <p>
    ...
    divergent replay all stop the logical Power. Assistant code does not receive a false value or catchable denial
    result. Team deletes the abandoned continuation and never retries the action automatically.
  </p>
  <p>
    If an earlier Power in the same Brain-requested batch already completed an external action, that action remains
...
<section class="guide-section" aria-labelledby="secret-title">
  ...
  <dl>
    ...
    <dd>Injected only for a Power that declared the Integration; reading it closes requests.</dd>
    ...
    <dd>Bounded title, description, labels, options, Assistant identity, Power identity, and expiry only.</dd>
  </dl>
</section>
